const getNextValueByInput = (input, counter) => {
  ...
  const nextValue = getNextValueByInput(pattern, ++counter);
  // console.log(pattern, nextValue)
  return +numbers[numbers.length - 1] + nextValue
}
...
const process = (inputs) => {
  return inputs.reduce((accum, curr) => {
    const nextValue = getNextValueByInput(curr, 0)
